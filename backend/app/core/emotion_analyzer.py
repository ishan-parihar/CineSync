...
class EmotionAnalyzer:
    ...
    def __init__(self, config: Dict, backend: str = "audio2emotion"):
        ...
        self.cache_enabled = config.get("emotion_analysis", {}).get(
            "cache_enabled", True
        )
        self.cache_dir = (
            Path(config.get("system", {}).get("cache_directory", "./cache"))
            / "emotions"
        )
    ...
    def _load_model(self) -> Any:
        ...
        if self.backend == "audio2emotion":
            if ort is None or librosa is None:
                logger.warning(
                    "Required libraries (onnxruntime, librosa) not installed. Using dummy model."
                )
                return None
    ...
    def analyze_audio(self, audio_path: str, transcript: Optional[str] = None) -> Dict:
        """
    ...
    def _segment_audio(self, audio: np.ndarray, sr: float) -> List[Dict]:
        """
    ...
    def _classify_audio2emotion(self, audio: np.ndarray, sr: int) -> Dict:
        ...
        if librosa is None or ort is None:
            ...
            logger.warning(
                "Librosa or onnxruntime not available, returning simulated emotion analysis"
            )
            ...
            result = {
                ...
                "secondary_emotions": [
                    {"name": "joy", "confidence": 0.3, "intensity": 0.3},
                    {"name": "fear", "confidence": 0.2, "intensity": 0.2},
                ],
        ...
        if self.model is None:
            raise EmotionAnalysisError(
                "Model not loaded properly. Check if model file exists and dependencies are installed."
            )
        ...
        padded_len = (
            (audio_len + chunk_size - 1) // chunk_size
        ...
        input_tensor: np.ndarray = audio[np.newaxis, :].astype(
            np.float32
        )  # Shape: (1, sequence_length)

        logger.info(
            f"Input tensor shape: {input_tensor.shape}, sequence_length={len(audio)} must be divisible by {chunk_size}"
        )
        ...
        if len(emotion_probs) != len(model_emotions):
            logger.warning(
                f"Model output shape {len(emotion_probs)} doesn't match expected emotions {len(model_emotions)}, using default mapping"
            )
        ...
        primary_emotion_name = (
            model_emotions[primary_idx]
            if primary_idx < len(model_emotions)
            else "neutral"
        )
        ...
        result = {
            ...
            "secondary_emotions": [
                {
                    "name": (
                        model_emotions[idx] if idx < len(model_emotions) else "neutral"
                    ),
    ...
    def _map_to_taxonomy(self, emotion_data: Dict) -> Dict:
        ...
        if standardized_name not in self.EMOTION_TAXONOMY:
            logger.warning(
                f"Unknown emotion '{standardized_name}', defaulting to 'trust'"
            )
    ...
    def _load_from_cache(self, cache_key: str) -> Optional[Dict]:
        ...
        if cache_file.exists():
            with open(cache_file, "r") as f:
                result: Optional[Dict] = json.load(f)
                return result
        return None
