...
class ContentOrchestrator:
    ...
    def generate_content(
        ...
        """
        self.logger.info(
            f"Starting content generation for {audio_path} with profile {profile_name}"
        )

        processing_log = {"start_time": self._get_timestamp(), "stages": {}}
        ...
        try:
            ...
            cinematographic_decisions = self.decision_engine.generate_shot_sequence(
                emotions
            )
            ...
            if output_path is None:
                output_filename = (
                    f"generated_{Path(audio_path).stem}_{profile_name}.mp4"
                )
    ...
    def _build_frame_sequences(
        self, shot_sequence: Any, emotions: Any, cinematic_mode: str = "balanced"
    ) -> list:
        """
        ...
        narrative_phase = (
            "setup"  # This would come from tension engine in full implementation
        )
        tension_score = (
            0.5  # This would come from tension engine in full implementation
        )
        ...
        for i, shot in enumerate(shot_sequence):
            ...
            emotion_segment = (
                ...
                else emotion_segments[0] if emotion_segments else {}
            )
            ...
            frame_sequences.append(
                {
                    ...
                    "shot_specification": shot.get(
                        "shot_specification",
                        {
                            "distance": shot.get(
                                "angle", "MCU"
                            ),  # Map angle to distance for compatibility
    ...
    def generate_batch(
        self, audio_files: list, profile_name: str, cinematic_mode: str = "balanced"
    ) -> list:
