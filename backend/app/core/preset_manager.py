...
class PresetManager:
                    ...
                    if base_config_file.exists():
                        ...
                        if emotions_dir.exists():
                            for emotion_dir in emotions_dir.iterdir():
                                ...
                                emotion_config["preset_name"] = (
                                    f"{character_dir.name}_{angle_dir.name}_{emotion_dir.name}"
                                )
                        ...
                        if config_file.exists():
                            ...
                            self.presets_cache[preset_key] = self._load_preset_config(
                                config_file
                            )
                            logger.debug(f"Loaded preset: {preset_key}")
                        else:
                            ...
                            if emotions_dir.exists():
                                for emotion_dir in emotions_dir.iterdir():
                                    ...
                                    if config_file.exists():
                                        preset_config = self._load_preset_config(
                                            config_file
                                        )
    ...
    def _load_preset_config(self, config_path: Path) -> Dict[str, Any]:
    ...
    def get_preset(self, preset_name: str) -> Dict[str, Any]:
        ...
        if preset_name not in self.presets_cache:
            raise ValueError(
                f"Preset '{preset_name}' not found. Available: {self.list_presets()}"
            )
    ...
    def create_preset_from_template(
        self, character_id: str, angle: str, description: str = ""
    ) -> str:
        ...
        template_config["metadata"]["created_date"] = str(
        ...
        try:
            ...
            if "background" in preset_config and "image" in preset_config["background"]:
                if not os.path.exists(preset_config["background"]["image"]):
                    logger.error(
                        f"Background missing: {preset_config['background']['image']}"
                    )
                    return False
