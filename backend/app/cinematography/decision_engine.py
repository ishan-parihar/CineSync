...
class CinematographicDecisionEngine:
    ...
    def _load_cinematography_rules(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                f"Cinematography rules file not found: {config_path}, using defaults"
            )
            return cast(
                Dict[str, Any],
                {
            ...
            logger.error(
                f"Invalid JSON in cinematography rules file: {config_path}, using defaults"
            )
            return cast(
                Dict[str, Any],
                {
    ...
    def generate_shot_sequence(
        self, emotion_analysis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        initial_shots = self.psycho_mapper.get_emotion_aware_shot_sequence(
            emotion_segments
        )
    ...
    def _validate_and_correct_shots(
        self,
        initial_shots: List[Dict[str, Any]],
        emotion_segments: List[Dict[str, Any]],
        tension_sequence: List[Dict[str, Any]],
    ) -> List[Dict[str, Any]]:
        """
        ...
        for shot, emotion, tension in zip(
            initial_shots, emotion_segments, tension_sequence
        ):
            combined_entry = {
                **shot,
        ...
        if validation_result["valid"]:
            ...
            logger.warning(
                f"Shot sequence has {len(validation_result['violations'])} violations, correcting..."
            )
    ...
    def _apply_grammar_corrections(
        self,
        initial_shots: List[Dict[str, Any]],
        emotion_segments: List[Dict[str, Any]],
    ...
    def _correct_distance_violations(
        ...
        if index > 0 and index < len(shots):
            ...
            progressive_distance = self._get_progressive_distance(
                prev_distance, curr_distance
            )
            shots[index]["shot_specification"]["distance"] = progressive_distance

    def _correct_angle_violations(
        self, shots: List[Dict[str, Any]], index: int
    ) -> None:
        """
    ...
    def _apply_temporal_smoothing(
        ...
        for i in range(1, len(shots)):
            ...
            if self._should_smooth_distance_change(previous_shot, current_shot):
                current_shot["shot_specification"]["distance"] = previous_shot[
                    "shot_specification"
                ]["distance"]

            # Smooth angle changes where appropriate
            if self._should_smooth_angle_change(
                previous_shot, current_shot, smoothing_factor
            ):
                current_shot["shot_specification"]["angle"] = previous_shot[
                    "shot_specification"
                ]["angle"]

            # Adjust duration to maintain rhythm
            current_shot["shot_specification"]["duration"] = (
                self._adjust_duration_for_rhythm(
    ...
    def _should_smooth_distance_change(
        self, prev_shot: Dict[str, Any], curr_shot: Dict[str, Any]
    ) -> bool:
        """
    ...
    def _should_smooth_angle_change(
        self,
    ...
    def _adjust_duration_for_rhythm(
        self,
        ...
        adjusted_duration = (
            1 - smoothing_factor
        ) * curr_duration + smoothing_factor * prev_duration
    ...
    def _determine_transition(
        self, shot: Dict[str, Any], index: int, all_shots: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        """
        # Default transition
        transition: Dict[str, Any] = {"type": "cut", "duration": 0.05}  # 50ms cut
        ...
        if index > 0:
            ...
            if curr_emotion != prev_emotion:
                ...
                prev_spec = cast(
                    Dict[str, Any], prev_shot.get("shot_specification", {})
                )
    ...
    def get_scene_breakdown(self, emotion_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        scene_breakdown = {
            "metadata": {
                ...
                "dominant_emotion": overall_sentiment.get(
            ...
            "tension_analysis": {
                "peaks": tension_peaks,
                "average_tension": (
                    sum(ts["tension_level"] for ts in tension_sequence)
            ...
            "cinematographic_notes": self._generate_cinematographic_notes(
                shot_sequence, tension_sequence
            ),
        }

        return scene_breakdown

    def _analyze_camera_movements(
    ...
    def _generate_cinematographic_notes(
        self,
        shot_sequence: List[Dict[str, Any]],
        tension_sequence: List[Dict[str, Any]],
    ) -> List[str]:
        ...
        high_tension_shots = [
            ...
            if s["shot_specification"]["distance"] in ["CU", "ECU"]
        ]
        if len(high_tension_shots) > 0:
            notes.append(
                f"Sequence includes {len(high_tension_shots)} close-up shots for emotional intensity"
            )

        # Note about angle consistency
        eye_level_shots = [
            s for s in shot_sequence if s["shot_specification"]["angle"] == "eye_level"
        ]
