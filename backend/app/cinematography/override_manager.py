...
class OverrideManager:
    ...
    def _load_override_rules(self) -> Dict[str, Any]:
        ...
        try:
            ...
            override_rules: Dict[str, Any] = rules.get(
                "override_rules",
                {
            ...
            logger.warning(
                f"Override rules config file not found: {config_path}, using defaults"
            )
            return {
                # Shot-level overrides
                "shot_distance_override": {
            ...
            logger.error(
                f"Invalid JSON in override rules file: {config_path}, using defaults"
            )
            return {
                # Shot-level overrides
                "shot_distance_override": {
    ...
    def apply_overrides(
        self, shot_sequence: List[Dict], emotion_analysis: Optional[Dict] = None
    ) -> List[Dict]:
        """
        ...
        logger.info(
            f"Applied {len([o for o in self.active_overrides.values() if o['applied']])} overrides"
        )
    ...
    def save_overrides_to_file(self, file_path: str) -> None:
        """
    ...
    def load_overrides_from_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Overrides loaded from {file_path}: {len(overrides)} overrides"
            )
            return True
        except Exception as e:
...
class CinematographicOverrideHelper:
    ...
    @staticmethod
    def create_closeup_override(
        override_id: str, segment_id: Optional[str] = None, duration: float = 1.5
    ) -> Dict:
        """
    ...
    @staticmethod
    def create_emotional_intensity_override(
        override_id: str, intensity: float, segment_id: Optional[str] = None
    ) -> Dict:
        """
    ...
    def create_transition_override(
        override_id: str, transition_type: str, segment_id: Optional[str] = None
    ) -> Dict:
        """
