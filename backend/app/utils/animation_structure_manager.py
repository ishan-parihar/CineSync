...
# Define a placeholder for when PresetManager is not available
_PresetManagerAvailable = False
PresetManager: Any = None

try:
    from ..core.preset_manager import PresetManager as _ActualPresetManager

    PresetManager = _ActualPresetManager
    _PresetManagerAvailable = True
except ImportError:
    pass
...
class AnimationStructureManager:
    ...
    def __init__(
        ...
        base_path: str = "profiles",
        preset_path: str = "assets/presets",
        config: Optional[AnimationFileConfig] = None,
    ):
        """
        ...
        self.logger.info(
            f"AnimationStructureManager initialized with base_path: {self.base_path}, preset_path: {self.preset_path}"
        )
    ...
    def _init_preset_manager(self) -> Any:
        """Initialize preset manager if available."""
        if _PresetManagerAvailable:
            try:
    ...
    def create_character(
        ...
        try:
            ...
            self.logger.info(
                f"Created character '{character_name}' with complete structure"
            )
    ...
    def _create_angle_structure(self, character_path: Path, angle_name: str) -> None:
    ...
    def add_angle(self, character_name: str, angle_name: str) -> bool:
        ...
        try:
            ...
            if angle_path.exists():
                self.logger.warning(
                    f"Angle '{angle_name}' already exists for character '{character_name}'"
                )
            ...
            self.logger.info(
                f"Added angle '{angle_name}' to character '{character_name}'"
            )
            print(
                f"Added angle '{angle_name}' to character '{character_name}' successfully"
            )
            return True

        except Exception as e:
            self.logger.error(
                f"Error adding angle '{angle_name}' to '{character_name}': {e}"
            )
            print(f"✗ Error adding angle '{angle_name}' to '{character_name}': {e}")
            return False

    def _add_angle_to_profile_config(
        self, character_name: str, angle_name: str
    ) -> None:
        ...
        if config_path.exists():
            ...
            if angle_name not in config["angles"]:
                ...
                config["angles"] = sorted(
                    list(set(config["angles"]))
    ...
    def add_emotion(
        self, character_name: str, angle_name: str, emotion_name: str
    ) -> bool:
        """
        ...
        try:
            ...
            if emotion_path.exists():
                self.logger.warning(
                    f"Emotion '{emotion_name}' already exists for '{character_name}/{angle_name}'"
                )
                print(
                    f"! Emotion '{emotion_name}' already exists for '{character_name}/{angle_name}'"
                )
            ...
            self.logger.info(
                f"Added emotion '{emotion_name}' to '{character_name}/{angle_name}'"
            )
            print(
                f"✓ Added emotion '{emotion_name}' to '{character_name}/{angle_name}'"
            )
            ...
            self.logger.error(
                f"Error adding emotion '{emotion_name}' to '{character_name}/{angle_name}': {e}"
            )
            print(
                f"✗ Error adding emotion '{emotion_name}' to '{character_name}/{angle_name}': {e}"
            )
    ...
    def add_viseme_image(
        self, character_name: str, angle_name: str, emotion_name: str, viseme_name: str
    ) -> bool:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Added viseme '{viseme_name}' to '{character_name}/{angle_name}/{emotion_name}'"
            )
            print(
                f"✓ Added viseme '{viseme_name}' to '{character_name}/{angle_name}/{emotion_name}'"
            )
            ...
            self.logger.error(
                f"Error adding viseme '{viseme_name}' to '{character_name}/{angle_name}/{emotion_name}': {e}"
            )
            print(
                f"✗ Error adding viseme '{viseme_name}' to '{character_name}/{angle_name}/{emotion_name}': {e}"
            )
    ...
    def list_emotions(self, character_name: str, angle_name: str) -> List[str]:
        ...
        emotions = []
        emotions_path = (
            self.base_path / character_name / "angles" / angle_name / "emotions"
        )
    ...
    def validate_character_animation_set(self, character_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Safely extract and type-check configuration values
            raw_angles = char_config.get("angles", self.config.angles)
            expected_angles: List[str] = []
            if isinstance(raw_angles, list):
                expected_angles = [
                    str(angle) for angle in raw_angles
                ]  # Ensure type safety
            else:
                expected_angles = [str(angle) for angle in self.config.angles]
            ...
            for angle in expected_angles:
                ...
                # Get expected emotions for this character
                raw_emotions = char_config.get("emotions", self.config.emotions)
                expected_emotions: List[str] = []
                if isinstance(raw_emotions, list):
                    expected_emotions = [
                        str(emotion) for emotion in raw_emotions
                    ]  # Ensure type safety
                else:
                    expected_emotions = [
                        str(emotion) for emotion in self.config.emotions
                    ]
                ...
                for emotion in expected_emotions:
                    ...
                    if not emotion_path.exists():
                        ...
                        # Get required visemes for this character
                        raw_visemes = char_config.get(
                            "visemes", self.config.required_files
                        )
                        required_visemes: List[str] = []
                        if isinstance(raw_visemes, list):
                            required_visemes = [
                                str(viseme) for viseme in raw_visemes
                            ]  # Ensure type safety
                        else:
                            required_visemes = [
                                str(viseme) for viseme in self.config.required_files
                            ]
                        ...
                        for viseme in required_visemes:
                            ...
                            if not viseme_path.exists():
                                missing_file = (
                                    f"angles/{angle}/emotions/{emotion}/{viseme}"
                                )
            ...
            validation_result["summary"] = {
                ...
                "completeness_percentage": (
                    (total_files_found / total_files_expected * 100)
            ...
            if validation_result["valid"]:
                ...
                self.logger.warning(
                    f"Character '{character_name}' validation failed - {len(validation_result['missing_files'])} missing files"
                )
                print(
                    f"[ERROR] Character '{character_name}' validation failed - {len(validation_result['missing_files'])} missing files"
                )
    ...
    def validate_all_characters(self) -> Dict[str, Any]:
        ...
        validation_results: Dict[str, Any] = {
            ...
            "invalid_characters": [],
            "details": {},
            "summary": {"total_files_expected": 0, "total_files_found": 0},
        }

        for character in all_characters:
            result = self.validate_character_animation_set(character)
            validation_results["details"][character] = result
            validation_results["summary"]["total_files_expected"] += result["summary"][
                "total_files_expected"
            ]
            validation_results["summary"]["total_files_found"] += result["summary"][
                "total_files_found"
            ]
    ...
    def validate_preset_integration(
        self, character_name: str, angle_name: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
                ...
                for emotion in self.config.emotions:
                    ...
                    if emotion_path.exists():
                        for viseme in self.config.viseme_images:
                            viseme_path = emotion_path / viseme
                            if not viseme_path.exists():
                                validation_result["missing_in_animation"].append(
                                    f"{emotion}/{viseme}"
                                )
                    else:
                        # Check if emotion exists in preset
                        validation_result["missing_in_animation"].append(
                            f"{emotion}/all_visemes"
                        )
            else:
            ...
            if preset_key in self.preset_manager.list_presets():
                try:
                    preset_config: Dict[str, Any] = self.preset_manager.get_preset(
                        preset_key
                    )
                    ...
                    for viseme_shape in preset_config.get("mouth_shapes", {}):
                        ...
                        for emotion in self.config.emotions:
                            ...
                            anim_viseme_path = (
                                angle_path / "emotions" / emotion / expected_anim_viseme
                            )
                            if not anim_viseme_path.exists():
                                missing_pair = f"{emotion}/{viseme_shape}"
                                if (
                                    missing_pair
                                    not in validation_result["missing_in_animation"]
                                ):
                                    validation_result["missing_in_animation"].append(
                                        missing_pair
                                    )
            ...
            if (
                not validation_result["preset_valid"]
                or not validation_result["animation_valid"]
                or validation_result["missing_in_animation"]
                or validation_result.get("missing_in_preset")
            ):
                validation_result["synchronized"] = False
    ...
    def validate_both_structures(self) -> Dict[str, Any]:
        ...
        try:
            ...
            overall_result["profiles_structure"][
                "validation_results"
            ] = profiles_validation
            overall_result["profiles_structure"]["summary"] = profiles_validation.get(
                "summary", {}
            )

            # Check if any profile characters are invalid
            overall_result["profiles_structure"]["valid"] = (
                profiles_validation["valid_characters"]
                == profiles_validation["total_characters"]
            )
        except Exception as e:
        ...
        try:
            if self.preset_manager:
                ...
                preset_validation_results: Dict[str, Any] = {}
                ...
                for preset in all_presets:
                    ...
                    preset_validation_results[preset] = {
                        "valid": is_valid,
                        "preset_info": (
                            self.preset_manager.get_preset(preset) if is_valid else None
                        ),
                    }
                ...
                overall_result["presets_structure"][
                    "validation_results"
                ...
                overall_result["presets_structure"]["valid"] = (
                    valid_presets == total_presets
                )
            else:
                overall_result["presets_structure"]["valid"] = False
                overall_result["presets_structure"][
                    "error"
                ] = "Preset manager not available"
        except Exception as e:
            overall_result["presets_structure"]["valid"] = False
        ...
        try:
            ...
            if self.preset_manager:
                ...
                preset_char_angle_pairs: List[tuple[str, str]] = []
                for preset in all_presets:
                ...
                preset_mapping: Dict[str, List[str]] = {}
                ...
                for char in profile_characters:
                    ...
                    if missing_in_presets:
                        detailed_comparison["missing_in_presets"].append(
                            {"character": char, "angles": list(missing_in_presets)}
                        )
                        for angle in missing_in_presets:
                            overall_result["cross_structure_validation"][
                                "differences"
                            ].append(
                    ...
                    if missing_in_profiles:
                        detailed_comparison["missing_in_profiles"].append(
                            {"character": char, "angles": list(missing_in_profiles)}
                        )
            ...
            if (
                overall_result["cross_structure_validation"]["differences"]
                or not overall_result["profiles_structure"]["valid"]
                or not overall_result["presets_structure"]["valid"]
                or detailed_comparison["content_differences"]
            ):
    ...
    def synchronize_structures(
        self,
        profile_to_preset: bool = True,
        preset_to_profile: bool = True,
            ...
            if profile_to_preset:
                ...
                for char in profile_characters:
                    ...
                    for angle in angles:
                        ...
                        if not preset_exists and create_missing:
                            ...
                            try:
                                new_preset_key = self.preset_manager.create_preset_from_template(
                                    char,
                                    angle,
                                    f"Created from profile structure for {char}/{angle}",
                                )
                                sync_results["created_structures"].append(
                                    f"preset:{new_preset_key}"
                                )
                                self.logger.info(
                                    f"Created preset {new_preset_key} from profile structure"
                                )
                            except Exception as e:
                                sync_results["errors"].append(
                                    f"Failed to create preset for {char}/{angle}: {str(e)}"
                                )
                        ...
                        if (
                            preset_to_profile or force_sync
                            ...
                            sync_result = self._sync_content_profile_to_preset(
                                char, angle, force_sync
                            )
                            if sync_result:
                        ...
                        if not char_exists and create_missing:
                            ...
                            try:
                                self.create_character(
                                    char, f"Created from preset {preset_key}"
                                )
                                sync_results["created_structures"].append(
                                    f"profile:{char}"
                                )
                                self.logger.info(
                                    f"Created profile character {char} from preset"
                                )
                            except Exception as e:
                                sync_results["errors"].append(
                                    f"Failed to create profile character {char}: {str(e)}"
                                )
                                sync_results["preset_to_profile"]["failed"] += 1
                                continue
                        ...
                        if char_exists or create_missing:
                            if angle not in self.list_angles(char):
                                try:
                                    ...
                                    self.logger.info(
                                        f"Added angle {angle} to character {char}"
                                    )
                                except Exception as e:
                                    sync_results["errors"].append(
                                        f"Failed to add angle {angle} to character {char}: {str(e)}"
                                    )
                                    sync_results["preset_to_profile"]["failed"] += 1
                                    continue

                        # Synchronize content from preset to profile structure
                        sync_result = self.synchronize_preset_to_animation(
                            char, angle, force_sync
                        )
                        if sync_result:
    ...
    def _sync_content_profile_to_preset(
        self, character_name: str, angle_name: str, force_sync: bool = False
    ) -> bool:
        """
        ...
        try:
            ...
            self.logger.debug(
                f"Content sync check from profile to preset for {character_name}/{angle_name}"
            )
            return True
    ...
    def create_consolidated_structure(
        self, target_path: str = "profiles_consolidated"
    ) -> bool:
        """
        ...
        try:
            ...
            for char in all_characters:
                ...
                profile_angles = (
                    set(self.list_angles(char)) if char in profile_characters else set()
                )
                    ...
                    for emotion in self.config.emotions:
                        ...
                        if profile_emotion_path.exists():
                            for viseme in self.config.required_files:
                                ...
                                if profile_viseme.exists() and (
                                    not target_viseme.exists() or False
                                ):  # Fixed undefined force_sync
                                    import shutil
                        ...
                        if (
                            self.preset_manager
                            and preset_key in self.preset_manager.list_presets()
                        ):
                            try:
                                preset_config = self.preset_manager.get_preset(
                                    preset_key
                                )
                                for viseme_shape, preset_viseme_path in preset_config[
                                    "mouth_shapes"
                                ].items():
                                    if os.path.exists(preset_viseme_path):
                                        ...
                                        if (
                                            not target_viseme.exists() or True
                                        ):  # Using True instead of force_sync for this context
                                            import shutil

                                            shutil.copy2(
                                                preset_viseme_path, target_viseme
                                            )
                            except Exception as e:
                                consolidation_report["errors"].append(
                                    f"Error copying preset visemes for {char}/{angle}: {str(e)}"
                                )
    ...
    def generate_structure_audit(self, include_details: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            profile_characters = self.list_characters()
            audit_result["profiles_structure"]["total_characters"] = len(
                profile_characters
            )
                                ...
                                if include_details:
                                    try:
                                        ...
                                        if (
                                            ...
                                            audit_result["cross_structure_analysis"][
                                                "file_size_analysis"
                                            ][str(viseme_path)] = {
                ...
                for preset in all_presets:
                    ...
                    if len(parts) == 2:
                        ...
                        preset_info = {
                            ...
                            "config": (
                                self.preset_manager.get_preset(preset)
                                if self.preset_manager.validate_preset(preset)
                                else None
                            ),
                        }

                        audit_result["presets_structure"]["presets"][
                            preset
                        ] = preset_info
            ...
            if profile_chars_set & preset_chars_set:
                for common_char in profile_chars_set & preset_chars_set:
                    ...
                    angle_coverage = {
                        "only_in_profiles": list(
                            profile_angles - preset_angles_for_char
                        ),
                        "only_in_presets": list(
                            preset_angles_for_char - profile_angles
                        ),
                        "in_both": list(profile_angles & preset_angles_for_char),
                    }

                    audit_result["cross_structure_analysis"]["completeness_comparison"][
                        common_char
                    ] = angle_coverage
    ...
    def export_structure_to_json(
        self, output_path: str = "animation_structure_export.json"
    ) -> bool:
        """
        ...
        try:
            ...
            for char in self.list_characters():
                ...
                for angle in self.list_angles(char):
                    angle_data: Dict[str, Any] = {"emotions": {}}

                    for emotion in self.list_emotions(char, angle):
                        emotion_data: Dict[str, Any] = {"visemes": []}
                        ...
                        for viseme in self.config.required_files:
                            ...
                            if viseme_path.exists():
                                ...
                                relative_path = viseme_path.relative_to(
    ...
    def import_structure_from_json(self, input_path: str) -> bool:
        ...
        try:
            ...
            for char_name, char_data in structure_data.get(
                "profiles_structure", {}
            ).items():
                # Create character
                self.create_character(
                    char_name, char_data["config"].get("description", "")
                )
                ...
                for angle_name, angle_data in char_data["angles"].items():
                    ...
                    for emotion_name, emotion_data in angle_data["emotions"].items():
                        ...
                        if emotion_name not in self.list_emotions(
                            char_name, angle_name
                        ):
                        ...
                        for viseme_info in emotion_data["visemes"]:
                            if viseme_info["exists"]:
                                ...
                                print(
                                    f"  Would copy: {viseme_info['name']} for {char_name}/{angle_name}/{emotion_name}"
                                )

            # Import presets structure
            if self.preset_manager and "presets_structure" in structure_data:
                for preset_name, preset_data in structure_data[
                    "presets_structure"
                ].items():
    ...
    def synchronize_preset_to_animation(
        self, character_name: str, angle_name: str, force_copy: bool = False
    ) -> bool:
        """
        ...
        try:
            ...
            if not angle_path.exists():
                print(
                    f"✗ Animation structure for '{character_name}/{angle_name}' does not exist"
                )
                return False
                ...
                for viseme_shape, preset_viseme_path in preset_config[
                    ...
                    if not animation_viseme_path.exists() or force_copy:
                        if os.path.exists(preset_viseme_path):
                            ...
                            print(
                                f"✓ Copied {viseme_shape} viseme to {animation_viseme_path}"
                            )
                        else:
                            print(
                                f"! Preset viseme {preset_viseme_path} does not exist"
                            )

            print(
                f"✓ Synchronized preset to animation for '{character_name}/{angle_name}'"
            )
            return True
    ...
    def export_structure_report(
        self, output_file: str = "animation_structure_report.json"
    ) -> bool:
    ...
    def import_from_existing_presets(
        self, preset_character: str, preset_angle: str, new_character: str
    ) -> bool:
        """
        ...
        try:
            ...
            if not character_path.exists():
                self.create_character(
                    new_character, f"Imported from preset {preset_key}"
                )
    ...
    def print_structure_tree(self, character_name: Optional[str] = None) -> None:
        ...
        if character_name:
            characters = (
                [character_name] if character_name in self.list_characters() else []
            )
        else:
    ...
    def migrate_to_new_structure(
        self, old_path: str = "assets/presets", new_path: str = "profiles"
    ) -> bool:
        """
                        ...
                        if angle_dir.is_dir():
                            ...
                            for emotion in self.config.emotions:
                                ...
                                if preset_config_path.exists():
                                    ...
                                    for viseme_shape, viseme_filename in preset_config[
                                        "mouth_shapes"
                                    ].items():
        ...
        try:
            ...
            if choice == "0":
                ...
                if char_name and angle_name and emotion_name and viseme_name:
                    manager.add_viseme_image(
                        char_name, angle_name, emotion_name, viseme_name
                    )
            elif choice == "5":
                ...
                if characters:
                    ...
                    for i, char in enumerate(characters, 1):
                        ...
                        print(
                            f"  {i}. {char} [{status}] - {completeness:.1f}% complete"
                        )
                else:
                    print("No characters found.")
            elif choice == "6":
                ...
                if char_name:
                    ...
                    print(
                        f"  Completeness: {result['summary']['completeness_percentage']:.1f}%"
                    )
                    ...
                    if result["missing_files"]:
                        ...
                        if len(result["missing_files"]) > 10:
                            print(
                                f"    ... and {len(result['missing_files']) - 10} more"
                            )
            elif choice == "9":
                ...
                print(
                    f"  Total Files Expected: {result['summary']['total_files_expected']}"
                )
                print(f"  Total Files Found: {result['summary']['total_files_found']}")
                completeness = (
                    (
                        result["summary"]["total_files_found"]
                        / result["summary"]["total_files_expected"]
                        * 100
                    )
                    if result["summary"]["total_files_expected"] > 0
                    else 0
                )
                print(f"  Overall Completeness: {completeness:.1f}%")
                if result["invalid_characters"]:
                    print(
                        f"  Invalid Character Names: {', '.join(result['invalid_characters'])}"
                    )
            elif choice == "10":
                char_name = input("Enter character name: ").strip()
                angle_name = input("Enter angle name: ").strip()
                if char_name and angle_name:
                    ...
                    print(
                        f"  Preset Valid: {'Yes' if result['preset_valid'] else 'No'}"
                    )
                    print(
                        f"  Animation Valid: {'Yes' if result['animation_valid'] else 'No'}"
                    )
                    print(
                    ...
                    if result["missing_in_preset"]:
                        print(
                            f"  Missing in Preset: {len(result['missing_in_preset'])}"
                        )
                    if result["missing_in_animation"]:
                        print(
                            f"  Missing in Animation: {len(result['missing_in_animation'])}"
                        )
            elif choice == "11":
                char_name = input(
                    "Enter specific character name (or press Enter for all): "
                ).strip()
                if char_name:
                ...
                output_file_input = input(
                    "Enter output file name (default: animation_structure_report.json): "
                ).strip()
                ...
                if preset_char and preset_angle and new_char:
                    manager.import_from_existing_presets(
                        preset_char, preset_angle, new_char
                    )
            elif choice == "14":
                char_name = input("Enter character name: ").strip()
                angle_name = input("Enter angle name: ").strip()
                force = (
                    input("Force overwrite existing files? (y/N): ").strip().lower()
                    == "y"
                )
                if char_name and angle_name:
                    manager.synchronize_preset_to_animation(
                        char_name, angle_name, force
                    )
            elif choice == "15":
                old_path = (
                    input("Enter old preset path (default: assets/presets): ").strip()
                    or "assets/presets"
                )
                new_path = (
                    input("Enter new profile path (default: profiles): ").strip()
                    or "profiles"
                )
                ...
                print(
                    f"Synchronized: {result['cross_structure_validation']['synchronized']}"
                )
                print(
                    f"Total Differences: {len(result['cross_structure_validation']['differences'])}"
                )
                if result["cross_structure_validation"]["differences"][
                    :5
                ]:  # Show first 5
                ...
                direction = input(
                    "Sync direction? (1) Preset to Profile, (2) Profile to Preset, (3) Both: "
                ).strip()
                ...
                print(
                    f"  Profile to Preset - Synced: {result['profile_to_preset']['synchronized']}, Failed: {result['profile_to_preset']['failed']}"
                )
                print(
                    f"  Preset to Profile - Synced: {result['preset_to_profile']['synchronized']}, Failed: {result['preset_to_profile']['failed']}"
                )
                ...
                target_path = input(
                    "Enter target path for consolidated structure (default: profiles_consolidated): "
                ).strip()
                if not target_path:
                    target_path = "profiles_consolidated"
                consolidation_result = manager.create_consolidated_structure(
                    target_path
                )
                print(f"Consolidation result: {consolidation_result}")
            elif choice == "19":
                include_details = (
                    input("Include file details in audit? (Y/n): ").strip().lower()
                    != "n"
                )
                audit_result = manager.generate_structure_audit(
                    include_details=include_details
                )
                output_file = input(
                    "Enter output file name for audit (default: structure_audit.json): "
                ).strip()
                if not output_file:
                ...
                export_path = input(
                    "Enter export file path (default: animation_structure_export.json): "
                ).strip()
                if not export_path:
                    export_path = "animation_structure_export.json"
                export_success = manager.export_structure_to_json(export_path)
                print(f"Export result: {export_success}")
            elif choice == "21":
                import_path = input("Enter import file path: ").strip()
                if import_path:
                    import_success = manager.import_structure_from_json(import_path)
                ...
                print(
                    f"Profiles Structure Valid: {validation_result['profiles_structure']['valid']}"
                )
                print(
                    f"Presets Structure Valid: {validation_result['presets_structure']['valid']}"
                )
                print(
                    f"Structures Synchronized: {validation_result['cross_structure_validation']['synchronized']}"
                )
                ...
                print(
                    f"  Total Files Expected: {prof_sum.get('total_files_expected', 0)}"
                )
                ...
                diff_count = len(
                    validation_result["cross_structure_validation"]["differences"]
                )
                print(f"  Differences Found: {diff_count}")
                if diff_count > 0:
                    print("  First 5 differences:")
                    for diff in validation_result["cross_structure_validation"][
                        "differences"
                    ][:5]:
                        print(f"    - {diff}")
                ...
                print(f"\nAudit Summary:")
                print(
                    f"  Total Characters: {audit_result['profiles_structure']['total_characters']}"
                )
                print(
                    f"  Total Angles: {audit_result['profiles_structure']['total_angles']}"
                )
                print(
                    f"  Total Emotions: {audit_result['profiles_structure']['total_emotions']}"
                )
                print(
                    f"  Total Visemes: {audit_result['profiles_structure']['total_visemes']}"
                )
