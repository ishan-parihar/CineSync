...
from typing import List, Dict, Any, Optional, Union, Collection
...
class AnimationStructureManager:
    ...
    def __init__(self, base_path: str = "profiles", preset_path: str = "assets/presets", config: Optional[AnimationFileConfig] = None):
        """
    ...
    def create_character(self, character_name: str, description: str = "", angles: Optional[List[str]] = None) -> bool:
        """
    ...
    def _add_angle_to_profile_config(self, character_name: str, angle_name: str) -> None:
    ...
    def validate_character_animation_set(self, character_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(raw_angles, list):
                expected_angles = [str(angle) for angle in raw_angles]  # Ensure type safety
            else:
            ...
            for angle in expected_angles:
                ...
                if isinstance(raw_emotions, list):
                    expected_emotions = [str(emotion) for emotion in raw_emotions]  # Ensure type safety
                else:
                    expected_emotions = [str(emotion) for emotion in self.config.emotions]
                ...
                for emotion in expected_emotions:
                    ...
                    if not emotion_path.exists():
                        ...
                        raw_visemes = char_config.get("visemes", self.config.required_files)
                        required_visemes: List[str] = []
                        if isinstance(raw_visemes, list):
                            required_visemes = [str(viseme) for viseme in raw_visemes]  # Ensure type safety
                        else:
                            required_visemes = [str(viseme) for viseme in self.config.required_files]
    ...
    def validate_preset_integration(self, character_name: str, angle_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if preset_key in self.preset_manager.list_presets():
                try:
                    preset_config: Dict[str, Any] = self.preset_manager.get_preset(preset_key)
            ...
            if (not validation_result["preset_valid"] or 
                ...
                validation_result.get("missing_in_preset")):
                validation_result["synchronized"] = False
                    ...
                    for emotion in self.config.emotions:
                        ...
                        if profile_emotion_path.exists():
                         for viseme in self.config.required_files:
                             ...
                             if profile_viseme.exists() and (not target_viseme.exists() or False):  # Fixed undefined force_sync
                                 import shutil
            ...
            for angle in self.list_angles(char):
                ...
                for i, emotion in enumerate(self.list_emotions(char, angle)):
                    ...
                    for j, viseme in enumerate(self.config.required_files):
                        ...
                        viseme_prefix = "            │   │   ├──" if not is_last_emotion else "                │   ├──"
        ...
        try:
            ...
            if choice == "0":
                ...
                if result['errors']:
                 ...
                 consolidation_result = manager.create_consolidated_structure(target_path)
