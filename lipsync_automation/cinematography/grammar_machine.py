...
class GrammarMachine:
    ...
    def _load_grammar_rules(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return cast(Dict[str, Any], rules.get('grammar_rules', {
    ...
    def _load_state_transitions(self) -> Dict[str, Any]:
        ...
        try:
            with open(config_path, 'r') as f:
                rules = cast(Dict[str, Any], json.load(f))
            return cast(Dict[str, Any], rules.get('state_transitions', {
                'establishing': {
    ...
    def _load_emotional_grammar(self) -> Dict[str, Any]:
        ...
        try:
            with open(config_path, 'r') as f:
                rules = cast(Dict[str, Any], json.load(f))
            return cast(Dict[str, Any], rules.get('emotional_grammar', {
    ...
    def validate_shot_sequence(self, shot_sequence: List[Dict[str, Any]], 
                              emotion_sequence: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
        """
        ...
        if validation_result['violations']:
            ...
            validation_result['score'] = max(0.0, 1.0 - (len(validation_result['violations']) * 0.2))
        else:
            validation_result['score'] = 1.0
        
        return validation_result
    
    def _check_distance_progression(self, shot_sequence: List[Dict[str, Any]]) -> List[str]:
        """
        ...
        for i in range(1, len(shot_sequence)):
            ...
            for forbidden_seq in self.grammar_rules['distance_progression']['forbidden_sequences']:
                if (prev_distance == forbidden_seq[0] and 
                    curr_distance == forbidden_seq[1]):
                    violations.append(
    ...
    def _check_angle_consistency(self, shot_sequence: List[Dict[str, Any]]) -> List[str]:
        """
        ...
        for i in range(1, len(shot_sequence)):
            ...
            allowed_angles = self.grammar_rules['angle_consistency']['angle_transition_rules'].get(
                prev_angle, []
            )
            
            if allowed_angles and allowed_angles != ['any'] and curr_angle not in allowed_angles:
                violations.append(
    ...
    def _check_emotional_grammar(self, shot_sequence: List[Dict[str, Any]], 
                                emotion_sequence: List[Dict[str, Any]]) -> List[str]:
        """
        ...
        for i in range(1, len(shot_sequence)):
            if i < len(emotion_sequence):
                ...
                prev_emotion_primary = cast(Dict[str, Any], prev_emotion_data.get('primary_emotion', {}))
                curr_emotion_primary = cast(Dict[str, Any], curr_emotion_data.get('primary_emotion', {}))
                ...
                for incompatible_pair in self.emotional_grammar['emotional_transition']['incompatible_pairs']:
                    if ((prev_emotion == incompatible_pair[0] and curr_emotion == incompatible_pair[1]) or
                        (prev_emotion == incompatible_pair[1] and curr_emotion == incompatible_pair[0])):
                        violations.append(
    ...
    def suggest_optimal_transition(self, current_shot: Dict[str, Any], 
        ...
        for state, transitions in self.state_transitions.items():
            for distance, props in transitions.items():
                ...
                transition_options.append({
                    'distance': distance,
                    'angle': self._suggest_compatible_angle(current_angle, distance),
        ...
        best_option = transition_options[0] if transition_options else {
        ...
        if target_emotion_name in self.emotional_grammar['emotional_containment']['high_intensity']:
            # For high intensity emotions, prefer closer shots
            if best_option['distance'] in ['LS', 'MLS']:
                best_option['distance'] = 'MCU'  # Move closer
        elif target_emotion_name in self.emotional_grammar['emotional_containment']['low_intensity']:
            # For low intensity emotions, maintain distance
            best_option['angle'] = 'eye_level'  # Maintain neutrality
        
        return best_option
    
    def _calculate_transition_score(self, current_shot: Dict, 
                                  next_distance: str, 
                                  target_emotion: Dict) -> float:
        """
    ...
    def _calculate_emotional_intensity_factor(self, emotion_name: str, 
    ...
    def generate_grammar_compliant_sequence(self, target_emotions: List[Dict[str, Any]],
                                          duration_constraints: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """
    ...
    def _generate_grammar_shot(self, emotion_data: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        stability_factor = 1.0 - emotion_data.get('intensity', 0.5)  # More intense = shorter
    ...
    def _calculate_timing(self, emotion_data: Dict[str, Any], 
                         duration_constraints: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def _correct_grammar_violations(self, shot_sequence: List[Dict[str, Any]],
    ...
    def _correct_distance_violations(self, sequence: List[Dict[str, Any]]) -> None:
        ...
        for i in range(1, len(sequence)):
            ...
            if not self._is_progressive_distance_change(prev_distance, curr_distance):
                ...
                sequence[i]['shot_specification']['distance'] = self._get_progressive_distance(
...
class CinematographicFSM:
    ...
    def __init__(self):
        self.states = {
            'establishing': {'entry_actions': ['wide_shot'], 'transitions': ['medium', 'closeup']},
            'medium': {'entry_actions': ['medium_shot'], 'transitions': ['closeup', 'establishing']},
            'closeup': {'entry_actions': ['closeup_shot'], 'transitions': ['medium', 'establishing']},
            'action': {'entry_actions': ['dynamic_shot'], 'transitions': ['closeup', 'medium']},
        }
