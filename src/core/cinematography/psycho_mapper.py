...
class PsychoCinematicMapper:
    ...
    def _load_tension_mappings(self) -> Dict:
        ...
        return {
            ...
            'critical': ['ECU', 'OTS']   # Extreme Close-up, Over-the-Shoulder for extreme tension
        }
    
    def select_shot(self, emotion_data: Dict, context: Dict = None) -> Dict:
        """
    ...
    def _determine_distance(self, emotion_data: Dict, context: Dict) -> str:
        ...
        if emotion_weight > 0.7:
            ...
            return emotion_prefs['primary_shots'][0] if emotion_prefs['primary_shots'] else 'CU'
        elif emotion_weight > 0.4:
            # Medium emotional intensity - use medium close shots
            if len(emotion_prefs['primary_shots']) > 1:
                return emotion_prefs['primary_shots'][1]
            else:
                return emotion_prefs['primary_shots'][0] if emotion_prefs['primary_shots'] else 'MCU'
        else:
    ...
    def _determine_duration(self, emotion_data: Dict, context: Dict) -> float:
        ...
        previous_shot = context.get('previous_shot', {})
        if previous_shot and previous_shot.get('distance') != 'ECU' and final_duration < 1.0:
            # Don't make shots too short if not extreme close-up
            final_duration = max(final_duration, 1.0)
        
        return min(final_duration, 5.0)  # Cap at 5 seconds
    ...
    def get_emotion_aware_shot_sequence(self, emotion_segments: List[Dict]) -> List[Dict]:
        """
