...
class VideoCompositor:
    ...
    def _render_with_background(self, concat_file: Path, audio_path: str,
        ...
        filter_complex = (
            f"[0:v]fps={self.video_config['fps']}[mouth]" # noqa
            f"[1:v]scale={self.video_config['resolution'][0]}:{self.video_config['resolution'][1]}[bg]" # noqa
            f"[bg][mouth]overlay={overlay_x}:{overlay_y}" # noqa
        )
