...
from lipsync_automation.cinematography import CinematographicDecisionEngine, TensionEngine, PsychoCinematicMapper, OverrideManager
        ...
        try:
            ...
            if isinstance(result, dict) and 'error' in result:
                return create_error_response(
                    ...
                    error_details=result if isinstance(result.get('error'), dict) else None,
                    request_id=request_id,
...
async def emit_emotion_segment_event(job_id: str, segment: Dict[str, Any], segment_index: int):
...
async def emit_shot_decision_event(job_id: str, emotion: str, shot_decision: Dict[str, Any]):
    ...
    event = {
        ...
        "reasoning": shot_decision.get("reasoning", "Cinematographic decision based on emotion analysis"),
...
async def emit_processing_stage_event(job_id: str, stage: str, progress: float, estimated_completion: Optional[str] = None):
...
@app.get("/")
async def read_root():
    return api_response_wrapper(lambda: {
        "message": "LipSyncAutomation Web API", 
        "status": "running"
    })()
...
def _get_system_performance_data():
    ...
    try:
        ...
        active_jobs = [job for job in all_jobs if job.get('status') in ['processing', 'queued']]
        ...
        error_rate = (len(failed_jobs) / total_finished_jobs * 100) if total_finished_jobs > 0 else 0
        
        # Recent performance (last 10 jobs)
        recent_jobs = sorted(completed_jobs, 
        ...
        resource_health = {
            ...
            "disk": {
                ...
                "status": "optimal" if all(part["percent"] < 70 for part in disk_partitions) else 
                         "warning" if all(part["percent"] < 90 for part in disk_partitions) else "critical",
        ...
        response = {
            ...
            "resource_utilization": {
                ...
                "disk": {
                    ...
                    "summary": {
                        ...
                        "healthy_partitions": sum(1 for p in disk_partitions if p["percent"] < 90),
                        "critical_partitions": sum(1 for p in disk_partitions if p["percent"] >= 90)
                    }
                },
                "network": {
                ...
                "process": {
                    ...
                    "file_descriptors": current_process.num_fds() if hasattr(current_process, 'num_fds') else None
                }
            },
            "processing_analytics": {
                ...
                "queue_performance": {
                    ...
                    "processing_capacity": "optimal" if queue_depth < 5 else "moderate" if queue_depth < 10 else "overloaded",
                    "estimated_wait_time": queue_depth * avg_processing_time if avg_processing_time > 0 and queue_depth > 0 else 0
                }
            },
            "system_health": {
                ...
                "health_checks": {
                    ...
                    "directories_writable": all(dir_info["writable"] for dir_info in directory_health.values()),
            ...
            "recommendations": _generate_performance_recommendations(
                cpu_percent, memory.percent, disk_partitions, 
                error_rate, queue_depth, cache_status
            ),
            "alerts": _generate_system_alerts(
                cpu_percent, memory.percent, disk_partitions,
                error_rate, dependencies_healthy, overall_status
            )
        }
...
def _generate_performance_recommendations(cpu_percent: float, memory_percent: float, 
                                        disk_partitions: List[Dict], error_rate: float,
                                        queue_depth: int, cache_status: Dict) -> List[Dict]:
...
def _generate_system_alerts(cpu_percent: float, memory_percent: float, 
                           disk_partitions: List[Dict], error_rate: float,
                           dependencies_healthy: bool, overall_status: str) -> List[Dict]:
...
def _sync_profile_manifest(profile_manager: ProfileManager):
    ...
    for profile_entry in manifest['profiles']:
        ...
        profile_config_path = profile_manager.profiles_dir / profile_name / "profile_config.json"
...
@app.get("/api/profiles/{profile_name}/angles/{angle_name}/emotions/{emotion_name}/visemes")
...
@app.post("/api/profiles/{profile_name}/angles/{angle_name}/emotions/{emotion_name}/visemes/{viseme_name}")
async def upload_viseme(profile_name: str, angle_name: str, emotion_name: str, viseme_name: str, file: UploadFile = File(...)):
...
@app.delete("/api/profiles/{profile_name}/angles/{angle_name}/emotions/{emotion_name}/visemes/{viseme_name}")
async def delete_viseme(profile_name: str, angle_name: str, emotion_name: str, viseme_name: str):
    ...
    try:
        ...
        profile_path = Path(config['system']['profiles_directory']) / profile_name
        viseme_path = profile_path / "angles" / angle_name / "emotions" / emotion_name / f"{viseme_name.upper()}.png"
        
        # Check if the file exists
        if not viseme_path.exists():
            return {"error": f"Viseme {viseme_name} does not exist for {profile_name}/{angle_name}/{emotion_name}"}
...
@app.get("/api/profiles/{profile_name}/angles/{angle_name}/emotions/{emotion_name}/visemes/{viseme_name}/image")
...
async def repair_profile_structure(profile_name: str, repair_data: Dict[str, Any]):
    ...
    try:
        ...
        results = profile_manager.create_missing_structure(profile_name, create_placeholders)
        return results
    except Exception as e:
        return {"error": str(e)}
...
async def create_angle(profile_name: str, angle_name: str, angle_data: Dict[str, Any] = {}):
...
async def create_emotion(profile_name: str, angle_name: str, emotion_name: str, emotion_data: Dict[str, Any] = {}):
...
async def copy_emotion(profile_name: str, copy_data: Dict[str, Any]):
    ...
    try:
        ...
        if not all([source_angle, target_angle, emotion_name]):
            return {"error": "source_angle, target_angle, and emotion_name are required"}
        ...
        source_emotion_path = profile_path / "angles" / str(source_angle) / "emotions" / str(emotion_name)
        target_emotion_path = profile_path / "angles" / str(target_angle) / "emotions" / str(emotion_name)
        
        if not source_emotion_path.exists():
            return {"error": f"Source emotion {emotion_name} does not exist in {source_angle}"}
    ...
    try:
        ...
        cinematography_rules = {
            "emotion_mappings": {
                ...
                "activation_status": {emotion: "active" for emotion in psycho_mapper.emotion_shot_mappings.keys()},
            ...
            "tension_mappings": {
                ...
                "activation_status": {category: "active" for category in ["low", "medium", "high", "critical"]},
                "explanations": {
...
async def get_cinematography_overrides():
    ...
    try:
        ...
        return {
            ...
            "override_types": list(set(override['type'] for override in active_overrides.values())),
            "timestamp": datetime.now().isoformat()
        }
...
def _validate_override_compatibility(override_type: str, override_value: Any, 
    ...
    if override_type in ['shot_distance_override', 'shot_angle_override', 'transition_override']:
        ...
        if not isinstance(override_value, (int, float)) or not (min_val <= override_value <= max_val):
            issues.append(f"Value for {override_type} must be between {min_val} and {max_val}")
    ...
    if override_type == 'shot_distance_override':
        ...
        if override_value == 'dutch' and conditions.get('emotion') in ['calm', 'peaceful']:
            notes.append("Dutch angles may create unnecessary tension for calm emotions")
    ...
    try:
        ...
        if enhanced_analysis["emotion_segments"]:
            ...
            enhanced_analysis["segment_statistics"] = {
                ...
                "durationweighted_emotion": _calculate_duration_weighted_emotion(segments)
            }
...
async def get_emotion_segments(job_id: str):
    ...
    try:
            ...
            enhanced_segment = {
                **segment,
                ...
                "acoustic_analysis": {
                    ...
                    "quality_metrics": _calculate_acoustic_quality_metrics(segment.get("acoustic_features", {})),
                    "speech_characteristics": _analyze_speech_characteristics(segment.get("acoustic_features", {}))
                },
                "cinematographic_suggestions": _generate_cinematographic_suggestions(segment)
            }
        ...
        response = {
            ...
            "alternative_emotion_suggestions": _generate_global_alternatives(emotion_segments),
            "processing_metadata": {
...
async def manual_emotion_adjustment(adjustment_data: Dict[str, Any]):
    ...
    try:
        ...
        updated_overall_sentiment = emotion_analyzer._calculate_overall_sentiment(current_segments)
...
async def get_job_shot_sequence(job_id: str):
    ...
    try:
        ...
        if job.get("status") != "completed":
            return {"error": f"Job {job_id} is not completed. Current status: {job.get('status')}"}
        ...
        if "cinematographic_decisions" in job_metadata:
            ...
            if audio_path and profile_name:
                try:
                    ...
                    emotions = emotion_analysis or orchestrator.emotion_analyzer.analyze_audio(audio_path)
                    cinematographic_decisions = orchestrator.decision_engine.generate_shot_sequence(emotions)
        ...
        for i, shot in enumerate(shot_sequence_data):
            ...
            decision_confidence = _calculate_shot_decision_confidence(shot, emotion_segment)
        ...
        response = {
            ...
            "sequence_analysis": {
                ...
                "total_duration": sum(shot["timing"]["duration"] for shot in enhanced_shot_sequence),
                "shot_type_distribution": _analyze_shot_type_distribution(enhanced_shot_sequence),
                "emotion_coverage": _analyze_emotion_coverage(enhanced_shot_sequence),
                "cinematic_consistency": _analyze_cinematic_consistency(enhanced_shot_sequence)
            },
            "recommendations": _generate_sequence_recommendations(enhanced_shot_sequence),
    ...
    try:
            ...
            enhanced_segment = {
                **segment,
                ...
                "acoustic_correlation": {
                    "features": segment.get("acoustic_features", {}),
                    "correlation_strength": _calculate_acoustic_emotion_correlation(segment),
                    "audio_quality_indicators": _assess_audio_quality_for_emotion(segment)
                },
                "transition_from_previous": transition_analysis,
                "cinematographic_implications": _analyze_cinematographic_implications(segment)
            }
            enhanced_segments.append(enhanced_segment)
        ...
        response = {
            ...
            "overall_emotion_analysis": {
                ...
                "emotional_intensity_curve": _calculate_intensity_curve(emotion_segments),
                "valence_arousal_trajectory": _calculate_valence_arousal_trajectory(emotion_segments)
            },
            "transition_analysis": {
                "total_transitions": len(emotion_segments) - 1 if len(emotion_segments) > 1 else 0,
                "transition_patterns": transition_patterns,
                "emotional_smoothness": _calculate_emotional_smoothness(emotion_segments),
                "dramatic_moments": _identify_dramatic_moments(emotion_segments)
            },
            "emotional_arc": emotional_arc,
            "segment_statistics": {
                "total_segments": len(enhanced_segments),
                "average_segment_duration": _calculate_average_segment_duration(emotion_segments),
                "emotion_distribution": _calculate_emotion_distribution(emotion_segments),
                "confidence_statistics": _calculate_confidence_statistics(emotion_segments)
            },
            "analysis_metadata": {
                "analysis_timestamp": datetime.now().isoformat(),
...
async def start_batch_processing(batch_data: Dict[str, Any]):
    ...
    try:
        ...
        for audio_file in audio_files:
            audio_path = audio_file.get("path") if isinstance(audio_file, dict) else audio_file
            ...
            valid_files.append({
                ...
                "custom_settings": audio_file.get("settings") if isinstance(audio_file, dict) else {}
            })
        
        if not valid_files:
            return {"error": "No valid audio files found", "validation_errors": validation_errors}
        ...
        for i, file_info in enumerate(valid_files):
            ...
            job_queue.append((priority, i, job_id, audio_path, final_profile, output_path, final_cinematic_mode))
...
async def get_batch_status(batch_id: str):
    ...
    try:
        ...
        completed_jobs = sum(1 for job in batch_info["jobs"].values() if job["status"] == "completed")
        failed_jobs = sum(1 for job in batch_info["jobs"].values() if job["status"] == "error")
        processing_jobs_count = sum(1 for job in batch_info["jobs"].values() if job["status"] == "processing")
        
        progress_percentage = (completed_jobs / total_jobs * 100) if total_jobs > 0 else 0.0
        ...
        batch_info["progress"]["queued"] = total_jobs - completed_jobs - failed_jobs - processing_jobs_count
        ...
        if processing_jobs_count > 0 and completed_jobs > 0:
            ...
            elapsed_time = (datetime.now() - datetime.fromisoformat(batch_info["timing"]["started"])).total_seconds()
            ...
            batch_info["timing"]["estimated_completion"] = estimated_completion.isoformat()
...
async def process_batch_async(batch_id: str, job_queue: list, config: Dict[str, Any]):
    ...
    try:
        ...
        for i, (priority, queue_pos, job_id, audio_path, profile_name, output_path, cinematic_mode) in enumerate(job_queue):
            try:
                ...
                batch_progress_event = {
                    ...
                    "percentage": (batch_info["progress"]["completed"] / batch_info["progress"]["total"]) * 100
                }
                await broadcast_websocket_event(batch_progress_event)
                
                # Process the individual job (this will emit detailed events)
                await process_job_async(job_id, audio_path, profile_name, output_path, cinematic_mode)
                ...
                if processing_jobs[job_id]["status"] == "completed":
                    ...
                    batch_job_complete_event = {
                        ...
                        "job_id": job_id,
                        "success": True,
                        "output_path": processing_jobs[job_id].get("result", {}).get("video_path")
                    }
        ...
        batch_complete_event = {
            ...
            "summary": {
                ...
                "completed": batch_info["progress"]["completed"],
                "failed": batch_info["progress"]["failed"],
                "success_rate": (batch_info["progress"]["completed"] / batch_info["progress"]["total"]) * 100
            }
        }
...
def _generate_alternative_shots(shot: Dict, primary_emotion: Dict) -> List[Dict]:
    ...
    if current_shot_type in shot_hierarchy:
        for alt_shot in shot_hierarchy[current_shot_type][:2]:  # Top 2 alternatives
            suitability_score = _calculate_shot_suitability(alt_shot, emotion_name, confidence)
            alternatives.append({
                "type": "shot_distance",
                "suggestion": alt_shot,
    ...
    if current_angle != "high_angle" and emotion_name in ["sadness", "fear", "vulnerability"]:
    ...
    if current_angle != "dutch" and emotion_name in ["surprise", "confusion", "tension"]:
...
def _calculate_shot_decision_confidence(shot: Dict, emotion_segment: Dict) -> float:
    ...
    primary_emotion = emotion_segment.get("primary_emotion", {})
    emotion_confidence = primary_emotion.get("confidence", 0.0) / 5.0  # Normalize to 0-1
    ...
    # Combine factors
    final_confidence = (base_confidence * 0.5) + (emotion_confidence * 0.3) + (signal_quality * 0.2)
...
def _generate_shot_reasoning(shot: Dict, primary_emotion: Dict) -> str:
    ...
    if emotion_name == "joy" and shot_type in ["CU", "ECU"]:
        reasoning_parts.append(f"Close-up shot emphasizes the {emotion_name} and creates intimacy")
    elif emotion_name == "sadness" and shot_type in ["MS", "LS"]:
        reasoning_parts.append(f"Wider shot provides emotional distance for {emotion_name}")
        ...
        reasoning_parts.append(f"Unconventional angle enhances the {emotion_name} and vulnerability")
    ...
    if confidence > 3.5:
        ...
        reasoning_parts.append("Conservative shot choice due to lower emotion confidence")
    
    return " | ".join(reasoning_parts) if reasoning_parts else "Standard cinematographic choice"
...
def _calculate_duration_weighted_emotion(segments: List[Dict]) -> Dict[str, float]:
    ...
    if total_duration > 0:
        return {emotion: duration / total_duration for emotion, duration in emotion_durations.items()}
    return {}
...
def _generate_alternative_emotions(segment: Dict) -> List[Dict]:
    ...
    for i, secondary in enumerate(secondary_emotions[:2]):  # Top 2 alternatives
        alternatives.append({
            ...
            "suitability_score": secondary.get("confidence", 0.0) / max(primary_emotion.get("confidence", 1.0), 1.0)
        })
    ...
    if speaking_rate > 1.2 and primary_emotion.get("name") not in ["anger", "excitement"]:
        contextual_alternatives.append({
            "emotion": "anger",
...
def _analyze_speech_characteristics(features: Dict) -> Dict:
    ...
    characteristics = {
        "tempo": "fast" if speaking_rate > 1.0 else "slow" if speaking_rate < 0.5 else "normal",
        "intensity": "high" if energy_level > 0.15 else "low" if energy_level < 0.05 else "medium",
        "pitch_range": "high" if pitch_mean > 600 else "low" if pitch_mean < 300 else "normal"
    }
...
def _generate_cinematographic_suggestions(segment: Dict) -> Dict:
    ...
    shot_suggestions = {
        ...
        "anticipation": {"distance": "CU", "angle": "low_angle", "movement": "slow_push"}
    }
...
def _analyze_feature_consistency(segments: List[Dict]) -> Dict:
    ...
    for feature in features:
        values = [seg.get("acoustic_features", {}).get(feature, 0.0) for seg in segments]
        if values:
    ...
    return {
        ...
        "analysis": "consistent" if overall_consistency > 0.7 else "variable" if overall_consistency > 0.4 else "inconsistent"
    }
...
def _generate_global_alternatives(segments: List[Dict]) -> Dict:
    ...
    alternatives = {
        ...
        "dominant_percentage": (sorted_emotions[0][1] / total_segments * 100) if sorted_emotions else 0,
        "alternative_arcs": []
    }
    ...
    if dominant_emotion == "trust":
        alternatives["alternative_arcs"].extend([
            {"name": "building_excitement", "suggestion": "Increase joy/anticipation in later segments"},
            {"name": "developing_tension", "suggestion": "Introduce fear/anger gradually"},
            {"name": "emotional_diversity", "suggestion": "Add more varied emotions throughout"}
        ])
    elif dominant_emotion == "sadness":
        alternatives["alternative_arcs"].extend([
            {"name": "hopeful_progression", "suggestion": "Transition towards joy/trust"},
            {"name": "deepening_emotion", "suggestion": "Intensify with fear/anger"},
            {"name": "resolution_arc", "suggestion": "Build towards neutral/trust conclusion"}
        ])
    
    return alternatives

def _validate_emotion_adjustment(segment: Dict, new_emotion: str, new_confidence: float, 
                               segment_index: int, all_segments: List[Dict]) -> Dict:
    ...
    if new_emotion not in valid_emotions:
        errors.append(f"Invalid emotion: {new_emotion}. Must be one of {valid_emotions}")
    
    # Validate confidence range
    if not isinstance(new_confidence, (int, float)) or not (0.0 <= new_confidence <= 5.0):
        errors.append("Confidence must be a number between 0.0 and 5.0")
    
    # Get valence/arousal for new emotion
    emotion_analyzer = EmotionAnalyzer({})
    valence, arousal = emotion_analyzer._emotion_to_valence_arousal(new_emotion, new_confidence)
    ...
    if new_confidence > 3.0 and segment_index < len(all_segments) - 1:
        ...
        if next_confidence < 1.0:
            notes.append("High confidence followed by low confidence may create emotional discontinuity")
...
def _validate_emotion_continuity(segments: List[Dict]) -> Dict:
    ...
    for i in range(1, len(segments)):
        ...
        if prev_emotion == curr_emotion:
            consecutive_count = 1
            j = i - 1
            while j >= 0 and segments[j].get("primary_emotion", {}).get("name") == curr_emotion:
                consecutive_count += 1
                j -= 1
            
            if consecutive_count > 3:
                warnings.append(f"Segment {i}: {consecutive_count} consecutive '{curr_emotion}' segments may create stagnation")
        ...
        if i >= 2:
            ...
            if len(set(emote_sequence)) == 3 and emote_sequence[0] == emote_sequence[2]:
                warnings.append(f"Segment {i}: Emotional whiplash detected with pattern {emote_sequence}")
    ...
    if len(segments) >= 3:
        ...
        if start_emotion == end_emotion and len(set(s.get("primary_emotion", {}).get("name", "") for s in segments)) == 1:
            warnings.append("Entire analysis uses single emotion - consider adding emotional variation")
    
    return {
        "warnings": warnings,
        "continuity_score": max(0.0, 1.0 - (len(warnings) / len(segments))) if segments else 1.0
    }
        ...
        while True:
            try:
                ...
                try:
                    ...
                    await handle_websocket_client_message(websocket, client_message, connection_id)
                ...
                print(f"WebSocket receive error: {type(receive_error).__name__}: {receive_error}")
                break
...
async def handle_websocket_client_message(websocket: WebSocket, message: Dict[str, Any], connection_id: str):
...
async def process_job_async(job_id: str, audio_path: str, profile_name: str, output_path: str, cinematic_mode: str):
    ...
    try:
        ...
        try:
            ...
            audio_features = orchestrator.audio_processor.extract_audio_features(audio_path)
            ...
            for i, segment in enumerate(emotion_segments):
                ...
                segment_progress = 25 + (35 * (i + 1) / len(emotion_segments)) if emotion_segments else 30
            ...
            for i, decision in enumerate(cinematographic_decisions):
                ...
                emotion_segment = emotion_segments[i] if i < len(emotion_segments) else {}
                emotion_name = emotion_segment.get("primary_emotion", {}).get("name", "neutral")
                
                await emit_shot_decision_event(job_id, emotion_name, decision)
                
                # Add reasoning if not present
                if "reasoning" not in decision:
                    decision["reasoning"] = _generate_shot_reasoning(decision, emotion_segment)
            
            # Emit tension analysis if available
            try:
                tension_sequence = orchestrator.decision_engine.tension_engine.calculate_sequence_tension(emotion_segments)
                # Create tension summary for event
                tension_summary = {
                    "overall_tension": "medium",
                    "tension_score": sum(t.get("tension_level", 0.5) for t in tension_sequence) / len(tension_sequence) if tension_sequence else 0.5,
                    ...
                    "dramatic_moments": [
                        ...
                        for i, t in enumerate(tension_sequence) if t.get("tension_level", 0.5) > 0.7
                    ]
                }
            ...
            result_metadata = {
                ...
                "processing_summary": {
                    ...
                    "dominant_emotion": _identify_dominant_emotion(emotion_segments)["dominant_by_frequency"],
                    "shot_variety": len(set(decision.get("angle", "MCU") for decision in cinematographic_decisions))
                }
            }
...
def _analyze_shot_type_distribution(shot_sequence: List[Dict]) -> Dict:
    ...
    for shot in shot_sequence:
        ...
        total_duration_by_type[shot_type] = total_duration_by_type.get(shot_type, 0) + duration
    ...
    return {
        ...
        "most_used_shot": max(shot_counts.items(), key=lambda x: x[1])[0] if shot_counts else "MCU"
    }
...
def _analyze_cinematic_consistency(shot_sequence: List[Dict]) -> Dict:
    ...
    return {
        ...
        "overall_consistency": (3.0 - (shot_type_changes + angle_changes + purpose_changes) / total_possible_changes) / 3.0
    }
...
def _generate_sequence_recommendations(shot_sequence: List[Dict]) -> List[Dict]:
    ...
    emotions = [shot.get("emotion_context", {}).get("primary_emotion", "neutral") for shot in shot_sequence]
...
def _analyze_emotion_transition(prev_segment: Dict, curr_segment: Dict) -> Dict:
    ...
    if prev_name == curr_name:
        ...
        opposing_emotions = {
            ("joy", "sadness"), ("anger", "trust"), ("fear", "disgust"), 
            ("surprise", "anticipation")
        }
        
        if (prev_name, curr_name) in opposing_emotions or (curr_name, prev_name) in opposing_emotions:
    ...
    return {
        ...
        "dramatic_level": "high" if intensity > 0.6 else "medium" if intensity > 0.3 else "low"
    }
...
def _calculate_emotion_intensity(segment: Dict) -> float:
    ...
    secondary_intensity = sum(emotion.get("confidence", 0.0) for emotion in secondary_emotions)
...
def _calculate_emotional_clarity(segment: Dict) -> float:
    ...
    strongest_secondary = max(secondary_emotions, key=lambda x: x.get("confidence", 0.0))
...
def _assess_audio_quality_for_emotion(segment: Dict) -> Dict:
    ...
    return {
        ...
        "reliability_for_emotion": "high" if quality_score > 0.7 else "medium" if quality_score > 0.4 else "low"
    }
...
def _analyze_cinematographic_implications(segment: Dict) -> Dict:
    ...
    base_recommendation = shot_recommendations.get(emotion_name, shot_recommendations["trust"])
    ...
    return {
        ...
        "cinematic_priority": "high" if intensity > 0.6 else "medium" if intensity > 0.3 else "low",
        "visual_energy": "high" if emotion_name in ["joy", "anger", "excitement"] else "low",
        "camera_movement_suitability": _assess_movement_suitability(emotion_name, intensity)
    }
...
def _analyze_emotional_transition_patterns(emotion_segments: List[Dict]) -> Dict:
    ...
    if len(emotion_segments) < 2:
        return {"pattern_type": "insufficient_data", "analysis": "Need at least 2 segments"}
    ...
    emotion_sequence = [seg.get("primary_emotion", {}).get("name", "neutral") for seg in emotion_segments]
    ...
    return {
        ...
        "transition_diversity": len(set(transition_counts)) / len(transition_counts) if transition_counts else 0
    }
...
def _analyze_emotional_arc(emotion_segments: List[Dict]) -> Dict:
    ...
    return {
        ...
        "emotional_direction": "positive" if overall_valence_trend > 0.1 else "negative" if overall_valence_trend < -0.1 else "neutral",
        "peak_emotion_moment": confidences.index(max(confidences)) if confidences else 0,
...
def _identify_dominant_emotion(emotion_segments: List[Dict]) -> Dict:
    ...
    for segment in emotion_segments:
        ...
        emotion_counts[emotion_name] = emotion_counts.get(emotion_name, 0) + 1
        emotion_durations[emotion_name] = emotion_durations.get(emotion_name, 0) + duration
        total_confidence[emotion_name] = total_confidence.get(emotion_name, 0) + confidence
    
    # Determine dominant by different metrics
    dominant_by_count = max(emotion_counts.items(), key=lambda x: x[1])[0] if emotion_counts else "neutral"
    dominant_by_duration = max(emotion_durations.items(), key=lambda x: x[1])[0] if emotion_durations else "neutral"
    dominant_by_confidence = max(total_confidence.items(), key=lambda x: x[1])[0] if total_confidence else "neutral"
    ...
    return {
        ...
        "consensus_dominant": dominant_by_count if dominant_by_count == dominant_by_duration == dominant_by_confidence else "mixed",
        "emotion_statistics": {
...
def _calculate_valence_arousal_trajectory(emotion_segments: List[Dict]) -> Dict:
    ...
    if len(trajectory_points) > 1:
        valence_range = max(p["valence"] for p in trajectory_points) - min(p["valence"] for p in trajectory_points)
        arousal_range = max(p["arousal"] for p in trajectory_points) - min(p["arousal"] for p in trajectory_points)
        ...
        valence_direction = "positive" if end_valence > start_valence else "negative" if end_valence < start_valence else "stable"
        arousal_direction = "increasing" if end_arousal > start_arousal else "decreasing" if end_arousal < start_arousal else "stable"
    else:
...
def _calculate_emotional_smoothness(emotion_segments: List[Dict]) -> float:
    ...
    for i in range(1, len(emotion_segments)):
        ...
        if prev_name == curr_name:
            ...
            conf_change = abs(curr_emotion.get("confidence", 0.0) - prev_emotion.get("confidence", 0.0))
...
def _calculate_emotion_distribution(emotion_segments: List[Dict]) -> Dict:
    ...
    return {
        ...
        "duration_percentages": {emo: (dur / total_duration * 100) for emo, dur in emotion_durations.items()},
        "segment_percentages": {emo: (count / total_segments * 100) for emo, count in emotion_counts.items()},
...
def _calculate_confidence_statistics(emotion_segments: List[Dict]) -> Dict:
    ...
    return {
        ...
        "max": max_confidence,
        "std_dev": std_dev,
        "confidence_stability": "high" if std_dev < 0.5 else "medium" if std_dev < 1.0 else "low"
    }

# Shot suitability functions
def _calculate_shot_suitability(shot_type: str, emotion_name: str, confidence: float) -> float:
    """Calculate suitability score for shot type with emotion"""
    # Define base suitability matrix
    suitability_matrix = {
        ("joy", "ECU"): 0.9, ("joy", "CU"): 1.0, ("joy", "MCU"): 0.8, ("joy", "MS"): 0.6,
        ("sadness", "ECU"): 0.5, ("sadness", "CU"): 0.6, ("sadness", "MCU"): 0.8, ("sadness", "MS"): 0.9,
        ("anger", "ECU"): 1.0, ("anger", "CU"): 0.9, ("anger", "MCU"): 0.7, ("anger", "MS"): 0.5,
        ("fear", "ECU"): 0.8, ("fear", "CU"): 0.9, ("fear", "MCU"): 0.7, ("fear", "MS"): 0.6,
        ("surprise", "ECU"): 0.9, ("surprise", "CU"): 1.0, ("surprise", "MCU"): 0.8, ("surprise", "MS"): 0.6,
        ("trust", "ECU"): 0.6, ("trust", "CU"): 0.7, ("trust", "MCU"): 0.9, ("trust", "MS"): 0.8,
        ("anticipation", "ECU"): 0.8, ("anticipation", "CU"): 0.9, ("anticipation", "MCU"): 0.8, ("anticipation", "MS"): 0.7
    }
...
def _calculate_angle_suitability(angle: str, emotion_name: str) -> float:
    ...
    angle_suitability = {
        ("joy", "eye_level"): 0.9, ("joy", "low_angle"): 0.7, ("joy", "high_angle"): 0.6, ("joy", "dutch"): 0.5,
        ("sadness", "eye_level"): 0.7, ("sadness", "low_angle"): 0.4, ("sadness", "high_angle"): 0.9, ("sadness", "dutch"): 0.6,
        ("anger", "eye_level"): 0.6, ("anger", "low_angle"): 0.9, ("anger", "high_angle"): 0.5, ("anger", "dutch"): 0.7,
        ("fear", "eye_level"): 0.6, ("fear", "low_angle"): 0.7, ("fear", "high_angle"): 0.8, ("fear", "dutch"): 0.9,
        ("surprise", "eye_level"): 0.8, ("surprise", "low_angle"): 0.6, ("surprise", "high_angle"): 0.7, ("surprise", "dutch"): 0.8,
        ("trust", "eye_level"): 0.9, ("trust", "low_angle"): 0.6, ("trust", "high_angle"): 0.7, ("trust", "dutch"): 0.4,
        ("anticipation", "eye_level"): 0.7, ("anticipation", "low_angle"): 0.8, ("anticipation", "high_angle"): 0.6, ("anticipation", "dutch"): 0.7
    }
...
def _analyze_angle_impact_change(current_angle: str, alternative_angle: str) -> str:
    ...
    dramatic_changes = [("eye_level", "dutch"), ("dutch", "eye_level"), ("high_angle", "low_angle"), ("low_angle", "high_angle")]
    moderate_changes = [("eye_level", "high_angle"), ("eye_level", "low_angle"), ("high_angle", "eye_level"), ("low_angle", "eye_level")]
    
    if (current_angle, alternative_angle) in dramatic_changes or (alternative_angle, current_angle) in dramatic_changes:
        return "dramatic_change"
    elif (current_angle, alternative_angle) in moderate_changes or (alternative_angle, current_angle) in moderate_changes:
...
@app.post("/api/profiles/{profile_name}/angles/{angle_name}/emotions/{emotion_name}/visemes/bulk-upload")
async def bulk_upload_visemes(profile_name: str, angle_name: str, emotion_name: str, files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        emotion_path = Path(profiles_directory) / profile_name / "angles" / angle_name / "emotions" / emotion_name
        ...
        for file in files:
            try:
                ...
                import re
                match = re.match(r'viseme_([A-HX])\.(jpg|jpeg|png|webp)$', file.filename, re.IGNORECASE)
                if not match:
...
@app.get("/api/profiles/{profile_name}/angles/{angle_name}/emotions/{emotion_name}/visemes/{viseme_name}/validate")
async def validate_viseme(profile_name: str, angle_name: str, emotion_name: str, viseme_name: str):
    ...
    try:
        ...
        viseme_path = Path(profiles_directory) / profile_name / "angles" / angle_name / "emotions" / emotion_name / f"{viseme_name}.png"
        ...
        try:
            with Image.open(viseme_path) as img:
                ...
                if width < 100 or height < 100:
                    issues.append(f"Image too small: {width}x{height}px (minimum 100x100)")
                    score -= 30
                elif width < 256 or height < 256:
                    issues.append(f"Image small: {width}x{height}px (recommended 256x256 or larger)")
                    score -= 15
                
                # Check aspect ratio
                aspect_ratio = width / height
                if aspect_ratio < 0.5 or aspect_ratio > 2.0:
                    issues.append(f"Unusual aspect ratio: {aspect_ratio:.2f} (recommended 0.8-1.2)")
                    score -= 10
                ...
                import random
                if random.random() < 0.1:  # 10% chance of finding quality issues for demo
        ...
        return {
            ...
            "dimensions": {"width": width, "height": height} if 'width' in locals() else None,
            "format": "PNG"
        }
...
@app.post("/api/profiles/{profile_name}/angles/{angle_name}/emotions/{emotion_name}/visemes/batch-delete")
async def batch_delete_visemes(profile_name: str, angle_name: str, emotion_name: str, viseme_data: Dict[str, List[str]]):
